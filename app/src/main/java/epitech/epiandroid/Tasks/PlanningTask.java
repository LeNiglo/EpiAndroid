...
public class PlanningTask extends AsyncTask<Void, Void, Boolean> {
    ...
    protected Boolean doInBackground(Void... params) {
        ...
        try {
			...
			String end= new SimpleDateFormat("yyyy-MM-dd").format(theFuture);
            ...
            try {
				...
				for (int i = 0; i < json.length(); ++i) {
					...
					if ((tmp = json.getJSONObject(i)) != null) {
						...
						// ajoute l'item dans la db
						Planning item = new Planning(tmp.getString("title"), dates);
						item.save();
					}
				}
